...
{
    //INTIALISATION 
    UnFichierTexte fichierTexte;
    bool finFichier = false;
    unsigned short int rang = 0;
    unsigned short int nbPair = 0;
    int valLue;
    associer(fichierTexte, nomSysteme);
    ouvrir(fichierTexte, consultation);

    //CORPS SOUS PROGRAMME
    while(finFichier == false)
    {
        lireMot(fichierTexte, valLue, finFichier);
        if(finFichier == true)
        {
            break;
        }
